...
public class Bwa implements Serializable {
	...
	*/
	...
	private String[] parseParameters(int alnStep) {
		...
		//B.E.S. - 20/2/2018 - changing to bwbble
		/*
		parameters.add("bwa");
		...
		if (algorithm.equals("mem") || algorithm.equals("bwasw")) {
			...
			if (this.pairedReads) {
				...
				LOG.info("Adding two FASTQ files to parameters");
			}
		}
		...
		*/
		...
		parameters.add("bwbble");
		parameters.add("align");
		parameters.add(this.indexPath);
		parameters.add(this.inputFile);
		parameters.add(this.inputFile + ".aln");
